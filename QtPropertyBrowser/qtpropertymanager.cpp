...
{
    typedef PropertyManagerPrivate::Data PrivateData;
    typedef QMap<const QtProperty *, PrivateData> PropertyToData;
    typedef PropertyToData::iterator PropertyToDataIterator;
...
public:
    ...
    struct Data
    {
			Data() : val(0), minVal(-9999999999999.999), maxVal(9999999999999.999), singleStep(0.001), decimals(3) {}
        double val;
        double minVal;
        double maxVal;
        double singleStep;
...
QString QtDoublePropertyManager::valueText(const QtProperty *property) const
{
    ...
    if (it == d_ptr->m_values.constEnd())
        return QString();
		QString result = QString::number(it.value().val, 'f', it.value().decimals);
		if (result.length() > 0)
			while (result.at(result.length() - 1) == QChar('0'))
				result.remove(result.length() - 1, 1);
		if (result.length() == 0)
			return QString();
		else if (result.at(result.length() - 1) == QChar('.'))
			result.remove(result.length() - 1, 1);
    return result;
}
