...
{
     PropertyToEditorListMap::iterator it = m_createdEditors.find(property);
...
*/
QtSpinBoxFactory::QtSpinBoxFactory(QObject *parent, bool isreadOnly)
...
/*!
    Creates a factory with the given \a parent.
*/
QtDoubleSpinBoxFactory::QtDoubleSpinBoxFactory(QObject *parent, bool isreadOnly)
        ...
        QtProperty *property, QWidget *parent)
{
    ...
    editor->setSingleStep(manager->singleStep(property));
		...
		QString stringVal = QString::number(manager->value(property), 'f', manager->decimals(property));
		int offset = 0;
		if (stringVal.length() > 0 && isReadOnly)
		{
			while (stringVal.at(stringVal.length() - 1) == QChar('0'))
			{
				stringVal.remove(stringVal.length() - 1, 1);
				offset++;
			}
		}
    editor->setDecimals(manager->decimals(property) - offset);
    editor->setRange(manager->minimum(property), manager->maximum(property));
    editor->setValue(manager->value(property));
    editor->setKeyboardTracking(false);
		editor->setReadOnly(isReadOnly);
		editor->setButtonSymbols(QAbstractSpinBox::NoButtons);
		QString styleSheet("QDoubleSpinBox{background-color:white;border:none;selection-color:white;");

		if (isReadOnly)
			styleSheet.append("color:#a0a0a4;");
		else
			styleSheet.append("color:black;");

		styleSheet.append("}");
		editor->setStyleSheet(styleSheet);
    connect(editor, SIGNAL(editingFinished()), this, SLOT(slotSetValue()));
    connect(editor, SIGNAL(destroyed(QObject *)),
                this, SLOT(slotEditorDestroyed(QObject *)));
    return editor;
}
        ...
        QtProperty *property, QWidget *parent)
{
    ...
    if (regExp.isValid()) {
		...
		QString styleSheet("QLineEdit{background-color:white;border: 1px solid white;selection-color:white;");
		if (m_isReadOnly)
			styleSheet.append("color:#6D6D6D;");
		else
			styleSheet.append("color:black;");
		styleSheet.append("}");
		editor->setStyleSheet(styleSheet);
