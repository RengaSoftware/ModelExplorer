            ...
            const QModelIndex &index) const
{
    ...
    if (m_editorPrivate) {
        ...
        if (property)
				{
            hasValue = property->hasValue();
						opt.palette.setBrush(QPalette::Inactive, QPalette::HighlightedText, QBrush(property->isModified() ? Qt::black : QColor(109, 109, 109)));
				}
    }
...
void QtTreePropertyBrowserPrivate::init(QWidget *parent)
{
    ...
    m_treeWidget->header()->setSectionResizeMode(QHeaderView::Stretch);
		m_treeWidget->setStyleSheet(QString("QTreeWidget{border:none}QTreeView::item:selected:!active{background-color:red;}"));
...
void QtTreePropertyBrowserPrivate::updateItem(QTreeWidgetItem *item)
{
    ...
    if (property->hasValue()) {
		...
		if (!property->isModified() && property->subProperties().length() == 0)
		{
			item->setTextColor(0, QColor(109, 109, 109));
			item->setTextColor(1, QColor(109, 109, 109));
		}
    ...
    bool wasEnabled = (item->flags() && Qt::ItemIsEnabled != 0);
